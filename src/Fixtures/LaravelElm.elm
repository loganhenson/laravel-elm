port module LaravelElm exposing (Errors, Page, Scroll, Viewports, decodeErrors, decodeViewports, page, pageWithMountAndSubscriptions, preserveScroll, receiveNewProps, saveScroll, sendScroll, setViewports)
...
type alias Page model msg =
    Program Value (Result Error model) msg
...
page :
    ...
    , newPropsMsg : Value -> msg
    }
    -> Program Value (Result Error { props : props, state : state }) msg
page withoutMount =
    pageWithMountAndSubscriptions
        { decodeProps = withoutMount.decodeProps
        , stateFromProps = withoutMount.stateFromProps
        , view = withoutMount.view
        , update = withoutMount.update
        , subscriptions = \_ -> receiveNewProps withoutMount.newPropsMsg
        , onMount = \_ -> Cmd.none
        }
...
pageWithMountAndSubscriptions :
    ...
    -> Program Value (Result Error { props : props, state : state }) msg
pageWithMountAndSubscriptions { decodeProps, stateFromProps, view, update, subscriptions, onMount } =
    Browser.element
        { init =
            \json ->
                let
